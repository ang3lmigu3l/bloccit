...
RSpec.describe Api::V1::PostsController, type: :controller do
  ...
  context "unauthenticated user" do
    ...
    it "PUT update returns http unauthorized" do
      put :update, id: my_post.id, post: {title: "Title", body: " Description"}
      expect(response).to have_http_status(401)
    end
    it "POST create returns http unauthenticated" do
      post :create, my_topic_id: my_topic.id, post: {title: "Title", body: "Description"}
      expect(response).to have_http_status(401)
    end
  ...
  context "unauthorized user" do
    ...
    it "PUT update returns http forbidden" do
      put :update, id: my_post.id, post: {title: "Topic Name", body: "Body"}
      expect(response).to have_http_status(403)
    end

    it "POST create returns http forbidden" do
      post :create, post: {title: "Topic Name", body: "Body"}
      expect(response).to have_http_status(403)
    end
  ...
  context " authenticated and authorized user" do
    ...
    describe "PUT update" do
      ...
      it "updates a post with correct attributes" do
